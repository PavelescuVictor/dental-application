...
export default {
    ...
    async mounted() {
        if (this.getSelectedOrderTypeEntry != "") {
            ...
            this.selectedType = this.types.filter((type) => {
                if (type.id === this.orderTypeEntry.type) return type;
            })[0];
            this.selectedStatus = this.status.filter((status) => {
                if (status.id === this.orderTypeEntry.status) return status;
            })[0];
            this.selectedColor = this.colors.filter((color) => {
                if (color.id === this.orderTypeEntry.color) return color;
            })[0];

            this.unitCount = this.getSelectedOrderTypeEntry.unitCount;
            this.warranty = this.getSelectedOrderTypeEntry.warranty;
            this.redo = this.getSelectedOrderTypeEntry.redo;
            this.paid = this.getSelectedOrderTypeEntry.paid;
        } else {
            this.alert = {
                type: "alert",
    ...
    methods: {
        ...
        handleSubmit(e) {
            ...
            let payload = {
                orderTypeEntryId: this.orderTypeEntry.id,
                data: {
            ...
            this.editOrderTypeEntry(payload)
                .then((response) => {
